...
export default function LoginPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:3000/organizadores/login', {
                email,
                senha,
            });
            ...
            <div className="w-full max-w-3xl bg-white rounded-[12px] shadow-lg border border-[#8FB339] flex flex-col">
                <div className="flex flex-col md:flex-row items-center p-10">
                    ...
                    <div className="flex-1 w-full">
                        <h2 className="text-xl font-bold font-inter text-[#4B5842] text-center md:text-left mb-4">Login</h2>
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            ...
                            <p className="text-sm text-right mt-1">
                                <a href="#" className="font-bold text-[#4B5842] hover:underline" onClick={openModal}>
                                    Esqueceu a senha?
                                </a>
                            </p>
                            ...
                            <p className="text-sm text-center mt-6 text-[#8FB339] font-inter font-medium">
                                Não tem uma conta? 
                                <a href="/register" className="font-bold text-[#4B5842] hover:underline ml-1">Cadastre-se</a>
                            </p>
                        </form>
                    </div>
                </div>
