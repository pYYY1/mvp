...
const Historico = () => {
  ...
  const [menuOpen, setMenuOpen] = useState(false); // Para controlar o menu flutuante
  ...
  useEffect(() => {
    const fetchCampeonatos = async () => {
      try {
        ...
        // Filtra campeonatos passados para o histórico
        const historicoList = data.filter((campeonato) => {
          const campeonatoData = new Date(campeonato.dataCampeonato);
          return campeonatoData < currentDate;
        });

        setCampeonatos(historicoList);
      } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    setModalOpen(false);
    setCampeonatoToDelete(null); // Limpar campeonato para excluir
  };

  const cancelDelete = () => {
    setModalOpen(false); // Fecha o modal
    setCampeonatoToDelete(null); // Limpa o campeonato selecionado
  };
    ...
    <div className="flex-grow">
      <BotaoVoltar />
        ...
        <div className="flex justify-center mt-4">
          <h1 className="text-3xl font-bold text-custom-green-2">Histórico de Campeonatos</h1>
        </div>
        ...
        <div className="flex flex-col items-center mt-10 w-5/6 mx-auto">
          {campeonatos.length === 0 ? (
            <p className="text-center text-xl">Nenhum campeonato passado encontrado.</p>
          ) : (
                            ...
                            <li>
                              <button
                                onClick={() => {
                                  ...
                                  setMenuOpen(false);  // Fechar o menu após copiar o link
                                }}
                            ...
                            <li>
                              <button
                                onClick={() => {
                                  ...
                                  setMenuOpen(false);  // Fechar o menu após clicar em Excluir
                                }}
